...
-- Policy: only authenticated users (admin) can read leads
-- Tighten to is_admin(auth.uid()) once E3 roles are implemented.
create policy "Authenticated users can read leads"
  on public.leads
  for select
  using (auth.uid() is not null);

-- Policy: only authenticated users (admin) can update leads
